...
For each requested support seed, this script samples exactly ``k`` source windows per class
...
--------
    ...
    python data/build_splits.py --datasets HHAR --shots 1 5 --support_seeds 0 1 2
"""
import argparse
import json
import os
...
def _validate_root(root, allow_unverified_root=False):
    ...
    manifest_path = os.path.join(root, "subject_split_manifest.json")
    if allow_unverified_root:
        return
    if not os.path.isfile(manifest_path):
        raise FileNotFoundError(
            f"{root} has no subject_split_manifest.json. Build the canonical arrays "
            "with data/build_subject_partitions.py, or pass --allow_unverified_root "
    ...
    if not (manifest.get("split_before_windowing")
            and manifest.get("subject_partitions_are_disjoint")):
        raise ValueError(
            f"{manifest_path} does not record a subject-disjoint split before windowing"
        )


def build(base_path, dataset, shots, support_seeds, allow_unverified_root=False):
    ...
    for support_seed in support_seeds:
        for k in shots:
            if k > smallest:
                print(
                    f"  support seed {support_seed}, {k}-shot: skipped; "
            ...
            if validation_idx.size == 0:
                raise ValueError(
                    f"support seed {support_seed}, {k}-shot leaves no source windows "
                    "for validation"
                )

            out = os.path.join(root, f"{k}-shot", f"support-seed-{support_seed}")
            ...
            print(
                f"  support seed {support_seed}, {k}-shot: {len(support_idx)} support windows "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--support_seeds", "--seeds", dest="support_seeds", type=int, nargs="+",
        default=list(SEEDS), help="support-set sampling seeds (legacy alias: --seeds)",
    )
    parser.add_argument(
        "--allow_unverified_root", action="store_true",
        help="accept externally prepared root arrays without the subject-split manifest",
    )
    args = parser.parse_args()
    for dataset_name in args.datasets:
        build(
            args.base, dataset_name, args.shots, args.support_seeds,
            args.allow_unverified_root,
        )
